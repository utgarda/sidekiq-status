...
describe Sidekiq::Status do
  ...
  context "keeps normal Sidekiq functionality" do
    ...
    it "retries failed jobs and still fails" do
      allow(SecureRandom).to receive(:hex).and_return(retry_and_fail_job_id)
      start_server do
        expect(capture_status_updates(3) { expect(RetryAndFailJob.perform_async).
            to eq(retry_and_fail_job_id) }).to eq([retry_and_fail_job_id] * 3)
