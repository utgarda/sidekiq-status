    ...
    module Storage
      ...
      def delete_and_unschedule(job_id, job_unix_time = nil)
        Sidekiq.redis do |conn|
          scan_options = { offset: 0, conn: conn,
                           start: (job_unix_time || '-inf'),
                           end: (job_unix_time || '+inf') }

          until (jobs = schedule_batch(scan_options)).empty?
      ...
      def schedule_batch(options)
        options[:conn].zrangebyscore 'schedule', options[:start], options[:end],
                                     limit: [options[:offset], BATCH_LIMIT]
      end
      ...
      def scan_scheduled_jobs_for_jid(scheduled_jobs, job_id)
        ...
        scheduled_jobs.find { |job_listing| job_listing.include?("\"jid\":\"#{job_id}") }
      end
